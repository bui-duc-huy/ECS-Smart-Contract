...
contract ClaimManager is PausableI, ERC725, ERC735 {
  ...
  /// @dev Requests the ADDITION or the CHANGE of a claim from an issuer.
        ...
        returns (uint256 claimRequestId)
    {
        ...
        if (!noApproval) {
            ...
            claimRequestId = execute(address(this), 0, msg.data);
    ...
    function getClaimId(address issuer, uint256 topic)
        ...
        returns (bytes32)
    {
        //  Doesn't allow multiple claims from the same issuer with the same type
        ...
        returns (bool)
    {
        if (_scheme == ECDSA_SCHEME) {
            ...
            if (issuer == address(this)) {
                ...
                if (issuer.doesContractImplementInterface(0xfccbffbc)) {
    ...
    modifier onlyManagementOrSelfOrIssuer(bytes32 _claimId) {
        ...
        if (issuer.doesContractImplementInterface(0xfccbffbc)) {
            ...
            require(ERC725(issuer).keyHasPurpose(addrToKey(tx.origin), EXECUTION_KEY));
        } else {
