...
contract IERC721 is IERC165 {
     ...
     */
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes32 keyAuth) public;
    /**
     * @dev Transfers a specific NFT (`tokenId`) from one account (`from`) to
     * another (`to`).
     *
     ...
     */
    function transferFrom(address from, address to, uint256 tokenId, bytes32 keyAuth) public;
    function approve(address to, uint256 tokenId, bytes32 keyAuth) public;
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data, bytes32 keyAuth) public;
}
...
contract ERC721 is ERC165, IERC721 {
    ...
    mapping (address => mapping (address => bool)) private _operatorApprovals;
     ...
     // Mapping from token ID to keyAuth
    mapping (uint256 => bytes32) private _keyAuth;
    ...
    function approve(address to, uint256 tokenId, bytes32 keyAuth) public {
        ...
        /*require(tx.origin == owner || isApprovedForAll(owner, tx.origin),
            "ERC721: approve caller is not owner nor approved for all"
        );*/
        _keyAuth[tokenId] = keyAuth;
    ...
    function setApprovalForAll(address to, bool approved) public {
        require(to != tx.origin, "ERC721: approve to caller");
    ...
    function transferFrom(address from, address to, uint256 tokenId, bytes32 keyAuth) public {
        ...
        require(_isApprovedOrOwner(tx.origin, tokenId) || _keyAuth[tokenId] == keyAuth);
     ...
     */
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes32 keyAuth) public {
        safeTransferFrom(from, to, tokenId, "", keyAuth);
    }
     ...
     */
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data, bytes32 keyAuth) public {
        require(_isApprovedOrOwner(tx.origin, tokenId) || _keyAuth[tokenId] == keyAuth, "ERC721: transfer caller is not owner nor approved");
        ...
        internal returns (bool)
    {
        ...
        bytes4 retval = IERC721Receiver(to).onERC721Received(tx.origin, from, tokenId, _data);
 ...
 */
contract ECSERC721 is ERC721, ERC721Enumerable, ERC721Metadata{
